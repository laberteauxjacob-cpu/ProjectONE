...
bool FONERegionalTraumaTest::RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("Subthreshold head hit applies the current 1.5 health multiplier"),Head->GetHealth(),89.5f);
    Packet=ONERegionalTestPacket(202,EONEHitRegion::Head,1,1);
    Head->ReceiveWeaponDamage(Packet);
    TestTrue(TEXT("Exact cumulative head trauma threshold cannot execute a surviving target"),Head->HasHead() && !Head->IsDead());
    TestEqual(TEXT("Crossing the trauma threshold still only applies 1.5 HP damage"),Head->GetHealth(),88.f);
    TestEqual(TEXT("Nonlethal head trauma produces no premature detachment"),Head->GetSeverCount(),0);
    Packet=ONERegionalTestPacket(203,EONEHitRegion::Head,60.f,0.f); Head->ReceiveWeaponDamage(Packet);
    TestTrue(TEXT("Later lethal head damage permits presentation at the accumulated trauma threshold"),!Head->HasHead() && Head->IsDead());
    TestEqual(TEXT("The eligible lethal head event creates exactly one detachment"),Head->GetSeverCount(),1);
    ...
    TestTrue(TEXT("Mixed nonlethal packet severs eligible arm while preserving head and life"),!Mixed->IsDead() && Mixed->HasHead() && !Mixed->HasLeftArm() && Mixed->HasRightArm());
    TestEqual(TEXT("Mixed health damage is 48 head plus 20 arm"),Mixed->GetHealth(),44.f);
    TestEqual(TEXT("Only the eligible arm region detaches"),Mixed->GetSeverCount(),1);
    ...
    AONEZombie* LethalMixed=Fixture.Spawn();
    if (!TestNotNull(TEXT("Lethal mixed-boundary target created"),LethalMixed)) return false;
    Packet=ONERegionalTestPacket(402,EONEHitRegion::Head,75.f,32.f);
    Packet.Get(EONEHitRegion::ArmLeft).AddPellet(50,50,FVector(0,-24,130),FVector::ForwardVector,-FVector::ForwardVector,TEXT("upperarm_r")); Packet.Finalize();
    LethalMixed->ReceiveWeaponDamage(Packet);
    TestTrue(TEXT("One lethal mixed packet resolves both eligible head and arm thresholds"),LethalMixed->IsDead() && !LethalMixed->HasHead() && !LethalMixed->HasLeftArm() && LethalMixed->HasRightArm());
    TestEqual(TEXT("Lethal mixed packet creates two detachments in one health transaction"),LethalMixed->GetSeverCount(),2);
    TestEqual(TEXT("Both lethal regional results share one damage transaction"),LethalMixed->GetDamageTransactionCount(),1);
