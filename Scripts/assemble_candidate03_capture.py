...
FFmpeg's path-bearing manifests stay local under Saved. Candidate02 is untouched.
...
def presentation_labels():
    modes = ('Standing fire and reload', 'Sideways fire', 'Diagonal sprint / fire', 'Movement / rapid aim')
    return {str(i): f'{"5.56 mm Carbine" if i < 8 else "12-Gauge Pump Shotgun"} / '
                   f'{"Bright" if i % 8 < 4 else "Dim"} / {modes[i % 4]}' for i in range(16)}
...
def prepare(args):
    ...
    phases = None
    if args.mode == 'presentation' or args.chapters:
    ...
    if args.mode == 'presentation' and observed != list(range(16)):
        raise ValueError('Presentation mode requires all 16 ordered contiguous phases (0 through 15).')
    labels = presentation_labels() if args.mode == 'presentation' else {}
    ...
    report = {'candidate': 'Candidate03', 'capture_kind': args.capture_kind, 'capture_mode': args.mode,
...
def main():
    ...
    p.add_argument('--mode', choices=('generic', 'presentation'), default='generic')
    ...
    p.add_argument('--replace', action='store_true', help='Replace intended C03 MP4/report only')
    ...
    if output.suffix.lower() != '.mp4' or any(part.lower() == 'candidate02' for part in output.parts):
        p.error('Choose an MP4 destination outside preserved Candidate02 artifacts.')
    ...
    if not args.validate_only and not args.replace and (output.exists() or sidecar.exists()):
        p.error('Output or provenance report exists; use --replace only for the intended C03 destination.')
    try:
    ...
    scratch_root = ROOT/'Saved/Candidate03/Assembly'
