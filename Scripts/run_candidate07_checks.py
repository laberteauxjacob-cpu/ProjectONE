...
def completion(log, marker):
    ...
    require(result.get('failures') == 0 and result.get('checks', 0) > 0, 'Actor completion reports failure or no checks')
    return result

def validate_fixture_completion(values, check):
    if check == 'profile':
        ...
        require(values.get('complete') == 1 and values.get('variants') == 3 and values.get('frames') == 0 and
                values.get('observed_frames', 0) > 0, 'Portability did not complete three recording-free variant episodes')
    else:
        ...
        require(check in ('physicality','encounter') and values.get('phases') == expected_phase and
                values.get('frames') == 0 and values.get('encounter') == int(check == 'encounter'),
...
def validate_portability(folder, done, log, labels):
    ...
    for key, expected in (('checks', done['checks']), ('failures', 0), ('variants_completed', 3),
                          ('observed_frames', done['observed_frames']), ('captured_frames', 0)):
        require(type(report.get(key)) in (int, float) and report[key] == expected, 'Portability count differs: ' + key)
    require(report.get('capture_requested') is False and report.get('capture_complete') is False, 'Portability report records media')
...
def main():
    ...
    ap.add_argument('check', choices=('profile', 'physicality', 'encounter', 'portability'))
    ...
    ap.add_argument('--timeout', type=int, default=240, help='Wall seconds; timed-out process is left running for inspection')
    args = ap.parse_args()
    ...
    require(30 <= args.timeout <= 360, 'Timeout must be between 30 and 360 seconds')
    ...
    result = {'schema': 'one07.private_runtime.v1', 'state': 'PREPARED', 'candidate': args.candidate,
              ...
              'settings': {'width':1600, 'height':900, 'max_fps':120, 'vsync':0, 'screen_percentage':100},
              'map': map_path,
              'recording': False, 'timeout_seconds': args.timeout,
    ...
    try:
        ...
        command += ['-'+flag, '-d3d12', '-RenderOffScreen', '-windowed', '-ResX=1600', '-ResY=900', '-ForceRes', '-unattended', '-nosplash',
                    '-ini:Engine:[Audio]:UnfocusedVolumeMultiplier=1.0', '-ExecCmds=t.MaxFPS 120,r.VSync 0,r.ScreenPercentage 100,t.MaxFPS,r.VSync,r.ScreenPercentage', '-abslog='+str(folder/'engine.log')]
        ...
        for key, path in roots.items():
            created = [p for p in path.iterdir() if p.is_dir() and p not in existing[key]] if path.exists() else []
        ...
        require(result['source_after'] == before, 'Source/content changed during runtime')
        ...
        validate_fixture_completion(done, args.check)
        result['assertions'] = {'driver':assertions(driver/'checks.txt',done['checks'])}
        if args.check == 'portability':
            result['portability'] = validate_portability(driver, done, log, result['assertions']['driver'])
        if args.check=='profile':
